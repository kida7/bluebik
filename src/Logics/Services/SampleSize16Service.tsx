...
function calculate(t: SampleSize16Request) {
  return doCalculation(t);
}

function doCalculation(t: SampleSize16Request) {
  var response: BaseResponse = { success: true, data: [], message: '' };
  var z_alpha, beta, z_beta, qa, qb, epsilon, nb, na, n_total;
  var result1,
    result1Value,
    result2,
    result2Value,
  ...
  nb = Helper.roundUp(
    ((t.pa * qa) / t.r + t.pb * qb) *
      Math.pow((z_alpha + z_beta) / (Math.abs(t.pa - t.pb) - t.delta), 2),
    0,
  );
  na = nb * t.r;
  n_total = na + nb;
